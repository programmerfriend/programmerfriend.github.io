...
In order to verify our setup, we also will create two web frontends. These frontends will consume the events sent via our Websocket implementation.
...
Truth be told, I have a feeling that compared to other frameworks, it is harder to get a plain Websocket connection in Spring Boot. But anyways, once you have done it - it is pretty straightforward.
...
When it receives an update, it will push this informations to a web frontend using a Websocket connection.

**TODO: Add image of architecture TODO**
...
* Either Web/Reactive Web - I have choosen Reactive Web
* WebSocket
...
In order to get them we need to create an Account at [https://developer.twitter.com/](https://developer.twitter.com/).
...
Since I don't want to spent a lot of time building a Web Client for the Twitter API just now - I hit the web and found [Twitter4J](http://twitter4j.org/en/index.html).
...
Twitter4J needs also the credentials in order to authenticate against the Twitter API.
...
My idea is to write a component which should receive the Tweets and transform them to Spring Internal Events.
...
Let's start with the component receiving the tweets and transforming them to an internal event.
...
It implements Twitter4J's `StatusListener` interface in order to act as a Listener for Tweets.
...
We need to setup the Twitter Stream and add our listener to it.
...
This code will setup our TwitterStream to track the terms `Elon Musk` and `Space X`.
Also, we have set the language to english and added our `twitterListener` as listener to the Stream.
...
The interface brings the `registerWebSocketHandlers` which allows to register our earlier implemented `WebsocketHandler` to the **/tweets**-Endpoint. We also allowed all origins (don't do this on production), since this makes things easier while developing from different ports and endpoints on a local machine.
...
This snippet establishes a Websocket connection and logs out each received message on the Chrome Console (should work similar on other browsers).
...
In order to finish our project we will execute the following steps:
...
* Call a method called `connect()` in order to connect and listen to the Websocket Events
...
There are probably better tutorials on writing a Webfrontend to consume a Websocket Connection.
...
When you have choosen a good topic in your `TwitterStreamConfig` you should see a few Tweets here and there.

## Recap

I hope you enjoyed the tutorial as much as I did while writing the code myself.
You created a Websocket Application which is streaming Tweets in real-time to your Webfrontend using a Websocket.

If you have any further questions, feel free to write a comment down below or just write me on [Twitter/@eiselems](https://twitter.com/eiselems).
